...
public class SessionCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		/*try {
			...
			//2020.5.12 by wanx 
			//如果非200，例如404，500，则request是org.apache.catalina.core.ApplicationHttpRequest类型，
			//request.getSession(true)会得到一个新的非共享的session，所以会重写cookie发到客户端，导致下次请求后的获取的非共享会话
			if (response.getStatus() != 200) {
				return true;
			}

		} catch (Exception e) {
			log.info("会话无效，请重新登录", e);
			request.setAttribute("message", "会话无效，请重新登录");
	...
	public void postHandle(HttpServletRequest request,
		...
		if (modelAndView == null) {
			log.debug("modelAndView为空，相应状态为：" + response.getStatus());
			return;
		}

		if (response.getStatus() == 500) {
			modelAndView.setViewName("base/5XX");
		} else if (response.getStatus() == 404) {
			modelAndView.setViewName("base/4XX");
		}
	}
